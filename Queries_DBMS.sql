...
-- Query 3 
Find Orders above 3000
Use Cust table to get Gender
Note: We need count of customers
...
select the customer name and gender whose names start or end with character 'A'
    (select cus_name and cus_gender from customer group by cus_id having
...
--QUERY 8 
select  cus_name, from customer where (type='name’ and 'gender='F' and quantity>0)
	    Select * from customer where cus_name starts and end with (A%);
